...
Reads <run_dir>/odom_liosam.csv and <run_dir>/odom_gt.csv (columns
stamp,x,y,z,qw,qx,qy,qz — recorded from /lio_sam/mapping/odometry and
/ground_truth/odom on the same replay, shared sim-time clock) and writes:

  <out>/lio_sam_est_<tag>.tum   LIO-SAM mapping trajectory (TUM)
  <out>/gt_gazebo_<tag>.tum     independent Gazebo model-state truth (TUM)
...
def main() -> int:
    ...
    gt = _load_rows(os.path.join(args.run, "odom_gt.csv"))

    os.makedirs(args.out, exist_ok=True)
    est_path = os.path.join(args.out, f"lio_sam_est_{args.tag}.tum")
    gt_path = os.path.join(args.out, f"gt_gazebo_{args.tag}.tum")
    src = f"exported from {args.run}"
    write_tum(est_path, est, f"LIO-SAM /lio_sam/mapping/odometry — {src}")
    write_tum(gt_path, gt,
              "ground truth: independent Gazebo model-state via /ground_truth/odom "
              f"(gazebo_truth_bridge, EVAL-06) — {src}")
